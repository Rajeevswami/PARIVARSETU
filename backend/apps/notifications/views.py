...
class NotificationViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if request.query_params.get("is_read") is not None:
            qs = qs.filter(is_read=request.query_params["is_read"].lower() == "true")
        return success_response(
            data=list(qs.values()[:100]), meta={"unread_count": qs.filter(is_read=False).count()}
        )
...
class TimelineViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if request.user.role != "family_admin" and hasattr(request.user, "member_profile"):
            qs = qs.filter(member=request.user.member_profile)
...
class AnnouncementViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        return success_response(
            data=list(Announcement.objects.filter(family_id=request.user.family_id).values())
        )

    def create(self, request):
        if request.user.role != "family_admin":
            raise PermissionDenied("Only family admins can create announcements.")
        a = Announcement.objects.create(
            family_id=request.user.family_id,
            created_by=request.user,
            title=request.data["title"],
            description=request.data["description"],
            visibility=request.data.get("visibility", "family"),
            start_date=request.data["start_date"],
            end_date=request.data.get("end_date") or None,
            attachment=request.FILES.get("attachment"),
        )
        return success_response(data={"id": str(a.id)}, status_code=201)
...
class SecurityViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if request.user.role != "family_admin":
            qs = qs.filter(member=getattr(request.user, "member_profile", None))
