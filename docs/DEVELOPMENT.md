...
`docker compose up -d db redis` locally. CI uses a fresh
`familynexus` / `familynexus_test` database.

OpenAPI is generated by `FamilyNexusAutoSchema` and
`apps.common.openapi.postprocess_schema`. Regenerate or inspect it at
`/api/v1/schema/`. Existing deployments rename Postgres and placeholder
emails with `scripts/migrate_parivarsetu_to_familynexus.sh` — see
`docs/REBRAND.md`.
